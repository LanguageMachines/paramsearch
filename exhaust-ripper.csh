      ...
      foreach a ( +freq -freq unordered )
        foreach n ( -n -\!n )
          foreach s ( 0.5 1.0 2.0 )
            foreach o ( 0 1 2 )
	      if ( $4 == "0" ) then
                ripper $1.$p -O$o -F$f -S$s -a$a $n > $1.$p.out 2>&1
	        echo f-$f=a-$a=n-$n=s-$s=o-$o >> ana-tmp.$p
                grep "Test" $1.$p.out | cut -c20- | cut -d"%" -f1 >> ana-tmp.$p
                rm $1.$p.out > /dev/null 2>&1
                rm $1.$p.hyp > /dev/null 2>&1
	      else
                foreach l ( 0.5 1.0 2.0 )
                  ripper $1.$p -L$l -O$o -F$f -S$s -a$a $n > $1.$p.out 2>&1
...
rm -f $1.ripper.bestsetting > /dev/null 2>&1
...
rm -f $1.ripper.log > /dev/null 2>&1
