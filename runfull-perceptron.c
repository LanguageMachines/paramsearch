/** runfull-perceptron - run a full experiment.

   syntax: runfull-perceptron <datafile> <testfile>

   assumes presence of <datafile>.perceptron.bestsetting
*/
...
int main(int argc, char *argv[])
{
  ...
  sprintf(fnaam,"%s.perceptron.bestsetting",
	  argv[1]);
  bron=fopen(fnaam,"r");
  if (bron==NULL)
    {
  ...
  fprintf(stderr,"best settings: %s %s\n",
	  dummy,settings);
  sprintf(cmd,"snow -train -I %s -F %s.net -P",
	  argv[1],argv[1]);
