    ...
    foreach j ( 0.5 1.0 2.0 )
      foreach c ( 0.001 0.005 0.01 0.05 1 5 10 50 100 500 1000 )
        foreach g ( 0.00025 0.0005 0.001 0.002 0.004 0.008 0.016 0.032 0.064 0.128 0.256 0.512 1.024 2.048 )
          svm_learn -c $c -j $j -t 2 -g $g $1.$p.data $1.$p.data.model >& /dev/null
          svm_classify $1.$p.test $1.$p.data.model >& $1.$p.test.out
          echo c-$c=j-$j=t-2=g-$g >> ana-tmp.$p
          grep "Accuracy" $1.$p.test.out | cut -d":" -f2 | cut -d"%" -f1 >> ana-tmp.$p
          rm $1.$p.test.out >& /dev/null
          rm $1.$p.test.model >& /dev/null
        end
        foreach d ( 1 2 3 4 5 )
          svm_learn -c $c -j $j -t 1 -d $d $1.$p.data $1.$p.data.model >& /dev/null
          svm_classify $1.$p.test $1.$p.data.model >& $1.$p.test.out
...
rm -f $1.svmlight.bestsetting >& /dev/null
